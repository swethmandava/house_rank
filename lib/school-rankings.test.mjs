...
test('maps combined school types to every covered level', () => {
  ...
  assert.equal(
    schoolCoversLevel(
      { schoolType: 'Elementary School', gradeLow: 0, gradeHigh: 8 },
      'm',
    ),
    true,
  );
...
test('loads the complete revised CSV schemas and current Alvarado rating', () => {
  ...
  const buenaVista = actualSchools.find(
    (item) => item.name === 'Buena Vista/ Horace Mann K-8',
  );
  assert.equal(buenaVista?.gradeLow, 0);
  assert.equal(buenaVista?.gradeHigh, 8);
  assert.equal(schoolCoversLevel(buenaVista, 'm'), true);
});

test('uses public schools by default while still returning private comparisons', () => {
  ...
  assert.equal(result.matchesByLevel[0].bestPrivate.name, 'Private option');
...
test('returns zero when a selected level has no considered school', () => {
  ...
  assert.equal(result.matchesByLevel[0].bestPrivate.name, 'Test school');
});
...
test('returns separate closest and best public and private schools', () => {
  const result = rankNearbySchools(
    home,
    { levelCodes: ['e'], sectors: ['public'], maxTravelMinutes: 30 },
    [
      school({ name: 'Closest public', greatSchoolsRating: 8 }),
      school({
        name: 'Best public',
        greatSchoolsRating: 10,
        longitude: -122.44,
      ...
      school({
        name: 'Closest private',
        publicVsPrivate: 'Private',
        greatSchoolsRating: null,
        greatSchoolsProfileUrl: null,
        privateStaffingProxyRating: 8,
      }),
      school({
        name: 'Best private',
        publicVsPrivate: 'Private',
  ...
  const level = result.matchesByLevel[0];
  assert.equal(level.closestPublic.name, 'Closest public');
  assert.equal(level.closestPrivate.name, 'Closest private');
  assert.equal(level.bestPublic.name, 'Best public');
  assert.equal(level.bestPrivate.name, 'Best private');
  assert.match(level.bestPrivate.ratingBand, /^Proxy Score /);
});
...
test('reranks with actual travel times and excludes options beyond the limit', () => {
  ...
  assert.equal(result.matchesByLevel[0].closestPublic.name, 'Nearby option');
  assert.equal(result.matchesByLevel[0].bestPublic.name, 'Nearby option');
  assert.equal(result.travelTimesEstimated, false);
});

test('uses the configured minimum rating to define suitable options', () => {
  const schools = [
    school({ name: 'Six out of ten', greatSchoolsRating: 6 }),
    school({
      name: 'Eight out of ten',
      greatSchoolsRating: 8,
      longitude: -122.449,
    }),
  ];
  ...
  assert.equal(result.matchesByLevel[0].suitableOptionCount, 1);
  assert.equal(
    result.matchesByLevel[0].closestSuitable.name,
    'Eight out of ten',
  );
  assert.equal(result.matchesByLevel[0].closestPublic.name, 'Eight out of ten');
  assert.equal(result.matchesByLevel[0].bestPublic.name, 'Eight out of ten');
  assert.deepEqual(
    result.matchesByLevel[0].topOptions.map((option) => option.name),
    ['Eight out of ten'],
  );
  assert.equal(result.matchesByLevel[0].minimumRating, 8);
});
