...
test('migrates a legacy houses array and removes it from the board', async () => {
  const originalFetch = globalThis.fetch;
  const requests = [];
  const legacyHouse = {
    id: 'legacy-house',
  ...
  globalThis.fetch = async (input, init) => {
    ...
    return Response.json(
      firestoreDocument({ houses: [legacyHouse], settings: {} }),
    );
  };

  try {
    const board = await getServerBoardState('board-123');
    assert.deepEqual(board.houses, [legacyHouse]);
    const houseWrite = requests.find(
      (request) =>
        request.method === 'PATCH' &&
        request.url.endsWith('/houses/legacy-house'),
    );
    assert.ok(houseWrite);
    const boardWrite = requests.find(
      (request) =>
        request.method === 'PATCH' && request.url.includes('fieldPaths=houses'),
    );
    assert.ok(boardWrite);
    assert.equal(JSON.parse(boardWrite.body).fields.houses, undefined);
  } finally {
    globalThis.fetch = originalFetch;
  }
});
