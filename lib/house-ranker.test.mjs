...
test('invalidating selected grades leaves unrelated results unchanged', () => {
  const house = {
    id: 'house-1',
    name: 'Example home',
    commute: '12 min average',
    ...
    notes: '',
    ratings: {
      commute: { auto: 4.5, rationale: 'Fast commute' },
      walkable: { auto: 4, rationale: 'Short walks' },
      view: { auto: 3.5, rationale: 'Some views' },
    },
  };

  const invalidated = invalidateAutoGrades(house, ['view']);

  assert.deepEqual(invalidated.ratings.view, { auto: null });
  assert.deepEqual(invalidated.ratings.commute, house.ratings.commute);
  assert.deepEqual(invalidated.ratings.walkable, house.ratings.walkable);
  assert.equal(invalidated.commute, house.commute);
  assert.equal(invalidated.nearby, house.nearby);
  assert.equal(invalidated.schools, house.schools);
});
